...
**Category:** Input | Combined date and time picker

## Constructor

```python
DateTimePicker(label: str = "")
```
...
| `add_theme_variants(*variants: pyxflow.components.constants.DateTimePickerVa...` | Add theme variants to the date time picker. |
...
| `get_value() -> datetime.datetime | None` |  |
| `get_width() -> 'str | None'` |  |
...
| `remove_theme_variants(*variants: pyxflow.components.constants.DateTimePickerVa...` | Remove theme variants from the date time picker. |
...
| `set_date_placeholder(placeholder: str)` |  |
...
| `set_label(label: str)` |  |
| `set_max(max_dt: datetime.datetime | None)` |  |
| `set_max_height(max_height: 'str | None')` |  |
| `set_max_width(max_width: 'str | None')` |  |
| `set_min(min_dt: datetime.datetime | None)` |  |
...
| `set_step(step: int | None)` | Set the time step in seconds (e.g., 1800 for 30 minutes). |
| `set_theme_name(theme_name: 'str | None')` | Set theme names, overwriting any previous ones. None removes all. |
| `set_time_placeholder(placeholder: str)` |  |
| `set_tooltip_text(text: 'str')` | Set the tooltip text. Creates a <vaadin-tooltip> child element. |
| `set_value(value: datetime.datetime | None)` |  |
| `set_visible(visible: 'bool')` | Set the visibility of the component. |
| `set_week_numbers_visible(visible: bool)` | Set whether ISO week numbers are shown in the date picker. |
