...
**Category:** Layout | Resizable split panel

## Constructor

```python
SplitLayout(args: Any)
```
...
| `add_splitter_drag_end_listener(listener)` | Add a listener for when the user stops dragging the splitter. |
| `add_theme_name(*theme_names: 'str')` | Add one or more theme names. |
| `add_theme_variants(*variants: pyxflow.components.constants.SplitLayoutVariant)` | Add theme variants to the split layout. |
| `add_to_primary(*components: pyxflow.core.component.Component)` | Add component(s) to the primary (left/top) area. |
| `add_to_secondary(*components: pyxflow.core.component.Component)` | Add component(s) to the secondary (right/bottom) area. |
...
| `get_orientation() -> pyxflow.components.constants.Orientation` |  |
| `get_primary_component() -> pyxflow.core.component.Component | None` |  |
| `get_secondary_component() -> pyxflow.core.component.Component | None` |  |
| `get_splitter_position() -> float | None` |  |
...
| `remove(component: pyxflow.core.component.Component)` | Remove a specific component (primary or secondary). |
| `remove_all()` | Remove all children. |
...
| `remove_theme_variants(*variants: pyxflow.components.constants.SplitLayoutVariant)` | Remove theme variants from the split layout. |
...
| `set_orientation(orientation: pyxflow.components.constants.Orientation)` | Set the orientation (HORIZONTAL or VERTICAL). |
| `set_primary_style(style_name: str, value: str)` | Set a CSS style on the primary component. |
| `set_secondary_style(style_name: str, value: str)` | Set a CSS style on the secondary component. |
| `set_size_full()` | Set both width and height to 100%. |
| `set_size_undefined()` | Remove both width and height. |
| `set_splitter_position(position: float)` | Set the splitter position as a percentage (0-100). |
