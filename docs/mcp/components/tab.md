...
**Category:** Navigation | Individual tab within Tabs
...
| `add_theme_variants(*variants: pyxflow.components.constants.TabVariant)` | Add theme variants to the tab. |
...
| `get_label() -> str` | Get the tab label text. |
...
| `is_selected() -> bool` | Check if this tab is selected. |
...
| `remove_theme_variants(*variants: pyxflow.components.constants.TabVariant)` | Remove theme variants from the tab. |
...
| `set_flex_grow(flex_grow: float)` | Set flex-grow for this tab. |
...
| `set_label(label: str)` | Set the tab label text. |
...
| `set_selected(selected: bool)` | Set the selected state (typically managed by Tabs, not called directly). |
