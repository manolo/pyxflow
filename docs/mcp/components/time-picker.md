...
**Category:** Input | Time selection

## Constructor

```python
TimePicker(label: str = "")
```
...
| `add_theme_variants(*variants: pyxflow.components.constants.TimePickerVariant)` | Add theme variants to the time picker. |
| `add_value_change_listener(listener: Callable)` |  |
| `blur()` | Remove focus from this component. |
| `close()` | Close the time picker overlay. |
...
| `open()` | Open the time picker overlay. |
...
| `remove_theme_variants(*variants: pyxflow.components.constants.TimePickerVariant)` | Remove theme variants from the time picker. |
...
| `set_label(label: str)` |  |
| `set_max(max_time: datetime.time | None)` |  |
| `set_max_height(max_height: 'str | None')` |  |
| `set_max_width(max_width: 'str | None')` |  |
| `set_min(min_time: datetime.time | None)` |  |
...
| `set_step(step: int | None)` | Set the step in seconds (e.g., 1800 for 30 minutes). |
...
| `set_value(value: datetime.time | None)` |  |
