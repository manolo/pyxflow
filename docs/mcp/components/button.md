...
**Category:** Action | Clickable button

## Constructor

```python
Button(text: str | Icon | Callable | None = "", on_click: Callable | Icon | None = None, icon: Icon | None = None)
```
...
| `add_theme_variants(*variants: pyxflow.components.constants.ButtonVariant)` | Add theme variants to the button. |
| `blur()` | Remove focus from this component. |
| `click()` | Trigger a click event programmatically (server-side). |
...
| `get_text() -> str` | Get the button text. |
...
| `is_autofocus() -> bool` |  |
| `is_disable_on_click() -> bool` | Check if the button disables itself when clicked. |
| `is_enabled() -> 'bool'` | Check if the component is enabled. |
| `is_icon_after_text() -> bool` |  |
...
| `remove_theme_variants(*variants: pyxflow.components.constants.ButtonVariant)` | Remove theme variants from the button. |
...
| `set_autofocus(autofocus: bool)` | Set whether the button should auto-focus on page load. |
| `set_class_name(class_name: 'str', add: 'bool' = True)` | Add or remove a CSS class name. |
| `set_disable_on_click(disable: bool)` | Set whether the button disables itself when clicked. |
...
| `set_icon(icon: 'Icon')` | Set the button icon (prefix slot). |
| `set_icon_after_text(after: bool)` | Place the icon after the text (suffix slot) instead of before (prefix). |
...
| `set_text(text: str)` | Set the button text. |
