...
**Category:** Input | Chat message input
...
| `add_submit_listener(listener: Callable)` | Add a listener called when the user submits a message. |
| `add_theme_name(*theme_names: 'str')` | Add one or more theme names. |
| `add_theme_variants(*variants: pyxflow.components.constants.MessageInputVari...` | Add theme variants to the message input. |
...
| `remove_theme_variants(*variants: pyxflow.components.constants.MessageInputVari...` | Remove theme variants from the message input. |
...
| `set_i18n(i18n: dict)` | Set the i18n localization object (button label, placeholder, etc.). |
