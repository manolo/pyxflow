...
**Category:** Input | Scrollable single-select list
...
Mixin methods: `set_read_only(bool)`, `is_read_only()`
...
| `set_item_enabled_provider(provider: Callable[[~T], bool])` |  |
| `set_item_label_generator(generator: Callable[[~T], str])` |  |
| `set_items(*items: ~T)` | Set the items for the list box. |
