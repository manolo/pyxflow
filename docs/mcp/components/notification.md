...
**Category:** Feedback | Toast notification

## Constructor

```python
Notification(text: str = "", duration: int = 0, position: Notification.Position | None = None, assertive: bool = False)
```
...
| Method | Description |
|--------|-------------|
| `add(*components: pyxflow.core.component.Component)` | Add components to the notification content. |
...
| `add_close_listener(listener: Callable)` | Add a listener for when the notification closes. |
| `add_focus_listener(listener)` | Add a focus event listener. |
| `add_open_listener(listener: Callable)` | Add a listener for when the notification opens. |
| `add_theme_name(*theme_names: 'str')` | Add one or more theme names. |
| `add_theme_variants(*variants: pyxflow.components.constants.NotificationVari...` | Add theme variants to the notification. |
...
| `is_opened() -> bool` | Check if the notification is open. |
| `is_visible() -> 'bool'` | Check if the component is visible. |
| `open()` | Open the notification. |
| `remove(*components: pyxflow.core.component.Component)` | Remove specific components from the notification content. |
| `remove_all()` | Remove all components from the notification content. |
...
| `remove_theme_variants(*variants: pyxflow.components.constants.NotificationVari...` | Remove theme variants from the notification. |
...
| `show(text: str, duration: int = 5000, position: 'Notification...` | Show a text notification. |
