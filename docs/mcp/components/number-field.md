...
**Category:** Input | Numeric input with step buttons

## Constructor

```python
NumberField(label: str = "")
```
...
| `get_value() -> float | None` | Get the current value. |
...
| `set_step(step: float)` | Set the step value for increment/decrement. |
| `set_step_buttons_visible(visible: bool)` | Show or hide the step buttons. |
...
| `set_value(value: float | None)` | Set the value. |
