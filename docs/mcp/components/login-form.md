...
**Category:** Authentication | Inline login form
...
| `add_forgot_password_listener(listener: Callable)` | Add a listener for the forgot password event. |
| `add_login_listener(listener: Callable)` | Add a listener for the login event. |
...
| `set_action(action: str)` | Set the form action URL. |
...
| `set_error(error: bool)` | Show or hide the error message. |
| `set_forgot_password_button_visible(visible: bool)` | Set whether the forgot password button is visible. |
...
| `set_i18n(i18n: dict)` | Set the i18n localization object (form labels, error messages, etc.). |
...
| `set_no_autofocus(no_autofocus: bool)` | Disable auto-focusing the username field on attach. |
