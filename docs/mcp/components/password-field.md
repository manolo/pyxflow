...
**Category:** Input | Password input with reveal toggle

## Constructor

```python
PasswordField(label: str = "")
```
...
| `is_reveal_button_visible() -> bool` | Check if the reveal button is visible. |
...
| `set_reveal_button_visible(visible: bool)` | Set whether the reveal button is visible. |
