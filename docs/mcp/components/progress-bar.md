...
**Category:** Display | Progress indicator
...
| `add_theme_variants(*variants: pyxflow.components.constants.ProgressBarVariant)` | Add theme variants to the progress bar. |
...
| `get_max() -> float` | Get the maximum value. |
| `get_max_height() -> 'str | None'` |  |
| `get_max_width() -> 'str | None'` |  |
| `get_min() -> float` | Get the minimum value. |
...
| `get_value() -> float` | Get the current value. |
| `get_width() -> 'str | None'` |  |
...
| `is_indeterminate() -> bool` | Check if the progress bar is indeterminate. |
...
| `remove_theme_variants(*variants: pyxflow.components.constants.ProgressBarVariant)` | Remove theme variants from the progress bar. |
...
| `set_indeterminate(indeterminate: bool)` | Set whether the progress bar is indeterminate. |
| `set_max(max_value: float)` | Set the maximum value. |
| `set_max_height(max_height: 'str | None')` |  |
| `set_max_width(max_width: 'str | None')` |  |
| `set_min(min_value: float)` | Set the minimum value. |
...
| `set_value(value: float)` | Set the value (between min and max). |
