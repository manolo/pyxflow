...
**Category:** Display | Chat message list
...
| `get_items() -> list[pyxflow.components.message_list.MessageListItem]` |  |
...
| `set_items(*items: pyxflow.components.message_list.MessageListItem)` | Set the message items. |
| `set_markdown(enabled: bool)` | Enable or disable markdown rendering in messages. |
...
```python
MessageListItem(text: str = "", time: Union = None, user_name: str = "", user_abbr: str = "", user_img: str = "", user_color_index: Union = None, theme: Union = None)
```

| Method | Description |
|--------|-------------|
| `append_text(text: str)` | Append text to this item and update the parent MessageList. |
| `to_dict() -> dict` |  |
