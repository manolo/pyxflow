...
**Category:** Input | Radio button group

## Constructor

```python
RadioButtonGroup(label: str = "")
```
...
| `add_theme_variants(*variants: pyxflow.components.constants.RadioGroupVariant)` | Add theme variants to the radio button group. |
...
| `remove_theme_variants(*variants: pyxflow.components.constants.RadioGroupVariant)` | Remove theme variants from the radio button group. |
...
| `set_items(*items: ~T)` | Set the items for the radio button group. |
