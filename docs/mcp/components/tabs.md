...
**Category:** Navigation | Horizontal/vertical tab bar
...
| Method | Description |
|--------|-------------|
| `add(*tabs: pyxflow.components.tabs.Tab)` | Add tabs to the component. |
...
| `add_tab_as_first(tab: pyxflow.components.tabs.Tab)` | Add a tab as the first tab. |
| `add_tab_at_index(index: int, tab: pyxflow.components.tabs.Tab)` | Add a tab at a specific index. |
| `add_theme_name(*theme_names: 'str')` | Add one or more theme names. |
| `add_theme_variants(*variants: pyxflow.components.constants.TabsVariant)` | Add theme variants to the tabs. |
...
| `get_orientation() -> str` | Get the tabs orientation. |
...
| `is_autoselect() -> bool` | Check if the first tab is auto-selected. |
| `is_enabled() -> 'bool'` | Check if the component is enabled. |
| `is_visible() -> 'bool'` | Check if the component is visible. |
| `remove(*tabs: pyxflow.components.tabs.Tab)` | Remove tabs from the component. |
| `remove_all()` | Remove all tabs. |
...
| `remove_theme_variants(*variants: pyxflow.components.constants.TabsVariant)` | Remove theme variants from the tabs. |
...
| `set_autoselect(autoselect: bool)` | Set whether the first tab should be auto-selected. |
...
| `set_orientation(orientation: str)` | Set the tabs orientation ('horizontal' or 'vertical'). |
...
| `update_selected_tab(changed_from_client=True)` | Called by client via publishedEventHandler when tab selection changes. |
