...
**Category:** Data | Virtualized list for large datasets
...
| `add_theme_variants(*variants: pyxflow.components.constants.VirtualListVariant)` | Add theme variants to the virtual list. |
...
| `remove_theme_variants(*variants: pyxflow.components.constants.VirtualListVariant)` | Remove theme variants from the virtual list. |
...
| `set_items(items: list[~T])` | Set the items for the virtual list. |
...
| `set_renderer(renderer: pyxflow.components.renderer.Renderer)` | Set the renderer for the virtual list. |
...
| `set_viewport_range(start: int, length: int)` | Called by client when the visible range changes. |
