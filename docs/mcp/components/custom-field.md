...
**Category:** Input | Wrapper for custom field compositions

## Constructor

```python
CustomField(label: str = "")
```
...
| Method | Description |
|--------|-------------|
| `add(*components: pyxflow.core.component.Component)` | Add child components to the custom field. |
...
| `add_theme_variants(*variants: pyxflow.components.constants.CustomFieldVariant)` | Add theme variants to the custom field. |
...
| `remove_theme_variants(*variants: pyxflow.components.constants.CustomFieldVariant)` | Remove theme variants from the custom field. |
