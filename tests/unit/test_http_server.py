...
class TestSessionCleanup:
    ...
    def setup_method(self):
        ...
        from pyxflow.server.http_server import _sessions, _upload_handlers
        _sessions.clear()
...
class TestUidlEncoder:
    ...
    def test_date_serialized_as_iso(self):
        import datetime
        from pyxflow.server.http_server import _UidlEncoder
        import json
...
class TestDevModeMetaTag:
    """Test pyxflow-views meta tag injection in dev mode."""
    ...
    def test_meta_tag_present_in_dev_mode(self):
        """Dev mode with views_module should inject pyxflow-views meta tag."""
        self._http._dev_mode = True
        self._http._views_module = "tests.views"
        html = self._http.get_index_html()
        assert '<meta name="pyxflow-views" content="tests.views">' in html

    def test_meta_tag_absent_in_production(self):
        """Production mode should NOT include pyxflow-views meta tag."""
        self._http._dev_mode = False
        self._http._views_module = "tests.views"
        html = self._http.get_index_html()
        assert "pyxflow-views" not in html
    ...
    def test_meta_tag_content_matches_module(self):
        ...
        html = self._http.get_index_html()
        assert '<meta name="pyxflow-views" content="demo.views">' in html
...
class TestUidlErrorHandling(AioHTTPTestCase):
    ...
    async def test_uidl_error_returns_app_error_not_500(self):
        ...
        with patch(
            "pyxflow.server.uidl_handler.UidlHandler.handle_uidl",
