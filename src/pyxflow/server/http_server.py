...
def get_bundle_directory() -> Path | None:
    ...
    2. Package-internal bundle (inside installed package / pyxflow source)
    ...
    if bundle_dir:
        ...
        if index_path.is_file():
            ...
            if _dev_mode and _views_module:
                html = html.replace(
                    '<base href="/">',
                    f'<base href="/">\n  <meta name="pyxflow-views" content="{_views_module}">'
                )
                html = html.replace(
                    '</body>',
...
<head>
  <meta charset="UTF-8" />
  <title>PyXFlow - Bundle Not Found</title>
</head>
<body>
...
async def handle_route(request: web.Request) -> web.Response:
    ...
    if ext in _STATIC_EXTENSIONS:
        ...
        from pyxflow.router import match_route
        if match_route(path):
            html = get_index_html()
            return web.Response(text=html, content_type="text/html")
